...
from quotes_client.interface import QuoteNotFound, QuoteServiceError, QuotesClientInterface

logger = logging.getLogger(__name__)


class Bot:
    def __init__(self, quote_service: QuotesClientInterface):
    ...
    async def _send_quote(self, update: Update, context: ContextTypes.DEFAULT_TYPE, quote_id: int):
        ...
        await context.bot.send_message(update.effective_message.chat_id, text=quote_text, reply_markup=keyboard)

    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        logger.info(f'Start received from {update.effective_chat.id}')
        await context.bot.send_message(chat_id=update.effective_chat.id, text=text_module.START_MESSAGE)

        await self._send_quote(update, context, 1)

    async def it_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        logger.info(f'IT received from {update.effective_chat.id}')
        text = update.message.text

        try:
            quote_id_str = text.split(maxsplit=1)[1]
        except IndexError:
            await update.message.reply_text(text_module.INVALID_IT_COMMAND)
            return
        ...
        await self._send_quote(update, context, quote_id)

    async def button(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        query = update.callback_query
        logger.info(f'button event received from {update.effective_chat.id}, data={query.data}')
